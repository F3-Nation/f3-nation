 ...
 * result into one array — for the handful of dropdowns/selectors that
 * genuinely need the complete list, not a page of it. Keeps every API
 * route safely bounded server-side (see pagination.ts) instead of relying
 * on an unbounded "omit both params" escape hatch that isn't safe for
 * every entity type.
 */
