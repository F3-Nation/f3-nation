...
planned_q AS (
    SELECT a.event_instance_id, a.user_id
    FROM pg.public.attendance a
    JOIN pg.public.attendance_x_attendance_types att ON a.id = att.attendance_id
    WHERE att.attendance_type_id = 2 AND a.is_planned = TRUE
)
SELECT ei.id, ei.org_id, ei.location_id, ei.series_id, ei.highlight,
       ...
       ei.description, ei.preblast, ei.meta, ei.created, ei.updated,
       ...
       o0.logo_url AS ao_logo_url, o0.website AS ao_website, o0.meta AS ao_meta,
       ...
       tc.all_types, ta.all_tags, q.user_id AS planned_q_user_id
...
LEFT JOIN planned_q q ON ei.id = q.event_instance_id
CROSS JOIN params p
WHERE ei.start_date >= p.as_of_date AND ei.is_active = TRUE
