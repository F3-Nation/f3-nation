...
def create_special_events_text(events: List[EventInstance], slack_settings_dict: dict, max_events: int = 10) -> str:
    text = ""
    if slack_settings_dict.get("calendar_config_special_days_out"):
        events = [e for e in events if (e.start_date - current_date_cst()).days <= slack_settings_dict.get("calendar_config_special_days_out")]  # noqa
