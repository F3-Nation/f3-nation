// Imported first in server.ts, before the `~/app` import, so Sentry's
// OpenTelemetry auto-instrumentation is in place before the libraries it
// patches (pg, http, etc.) are loaded — ESM resolves imports in source order,
// so initializing Sentry after importing `app` would be too late.
