...
- **AC-13** — GIVEN status aggregation in `packages/api` receives valid and supported contract JSON WHEN evaluating a contract monitor THEN API result status equals body `status`, and homepage displays check-level degradation/failure details and last updated timestamp. Contract version is additionally shown when the `/status?detail` query parameter is present.
