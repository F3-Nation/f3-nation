...
- The `main` branch ruleset's required status checks and `check-regexp` in the deploy workflows (`_deploy-cloudrun.yml`, `_deploy-cloudrun-job.yml`, `deploy-homepage.yml`) are two independently-maintained lists — renaming or adding a required check needs both updated by hand, and they can drift apart if one is. They already have: the ruleset currently requires 8 checks (`format-check`, `lint`, `typecheck`, `build`, `test-coverage`, `security-audit`, `lint-title`, `db-schema-sync`), but `check-regexp` still only waits on the original 5 (`build`, `lint`, `typecheck`, `format-check`, `test-coverage`) — deploys are not currently gated on `security-audit`, `lint-title`, or `db-schema-sync` passing.
