...
1. Run `pnpm ci:local` locally — it chains the same steps CI runs — then make sure all Checks pass once pushed. Catching issues locally is faster than waiting on CI.
   - The test step can rewrite `vitest.config.ts` coverage thresholds in the apps it ran (they ratchet upward). That's expected — commit the updated config with your change, and never lower a threshold by hand. See [AGENTS.md](../AGENTS.md) for the rules.
1. Mark the PR as **Ready for review**. At this point, a number of AI bots will run and provide feedback (see "Use of AI" below).
   - The bots don't review drafts. CodeRabbit posts a "Draft PR not reviewed" notice while your PR is still a draft — that's normal; the reviews arrive once you mark it ready.
