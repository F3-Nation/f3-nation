...
BEGIN
  ...
  -- Escape delimiters in composite text keys; integer keys retain their format.
  row_j := COALESCE(new_j, old_j);
  SELECT string_agg(
    CASE WHEN cardinality(pk_cols) > 1
...
-- API keys are bearer credentials; retain rotation history without their values.
SELECT audit.enable_tracking('public.api_keys'::regclass, ARRAY['updated'], ARRAY['key']);
...
-- Migration 0028 adds the primary keys needed by the remaining Codex tables.

SELECT audit.enable_tracking('codex.admins'::regclass, ARRAY['updated_at']);
SELECT audit.enable_tracking('codex."references"'::regclass, ARRAY['updated']);
SELECT audit.enable_tracking('codex.entries'::regclass, ARRAY['updated_at']);
SELECT audit.enable_tracking('codex.entry_references'::regclass, ARRAY['updated_at']);
SELECT audit.enable_tracking('codex.entry_tags'::regclass);
SELECT audit.enable_tracking('codex.tags'::regclass, ARRAY['updatedAt']);
SELECT audit.enable_tracking('codex.user_submissions'::regclass, ARRAY['updated_at']);
