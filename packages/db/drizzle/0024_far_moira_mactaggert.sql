...
ALTER TABLE "auth"."better_auth_oauth_refresh_token" ADD CONSTRAINT "better_auth_oauth_refresh_token_user_id_better_auth_user_id_fk" FOREIGN KEY ("user_id") REFERENCES "auth"."better_auth_user"("id") ON DELETE cascade ON UPDATE no action;--> statement-breakpoint
ALTER TABLE "auth"."better_auth_user" ADD CONSTRAINT "better_auth_user_f3_user_id_fkey" FOREIGN KEY ("f3_user_id") REFERENCES "public"."users"("id") ON DELETE cascade ON UPDATE no action;--> statement-breakpoint
ALTER TABLE "auth"."better_auth_user" ADD CONSTRAINT "better_auth_user_f3_user_id_key" UNIQUE("f3_user_id");--> statement-breakpoint
