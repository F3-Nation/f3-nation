...
describe("Event Instance Router", () => {
  ...
  describe("byId", () => {
    ...
    it("should accept includeSlackChannelId as the string \"true\"", async () => {
      const session = await createAdminSession();
      await mockAuthWithSession(session);
    ...
    it("should treat a blank AO org meta slack channel id as unset", async () => {
      const session = await createAdminSession();
      await mockAuthWithSession(session);
      ...
      const ao = await createTestAO(region.id, { slack_channel_id: "   " });
      if (!ao) return;
    ...
    it("should trim the AO org meta slack channel id when used as a fallback", async () => {
      const session = await createAdminSession();
      await mockAuthWithSession(session);
      ...
      expect(getSlackChannels(result)).toEqual({
        preblast: { channelId: "C_AO_TRIMMED", source: "ao_org_meta" },
        backblast: { channelId: "C_AO_TRIMMED", source: "ao_org_meta" },
      });
    });

    it("should fall back to AO meta when the region destination is not specified_channel", async () => {
      const session = await createAdminSession();
      await mockAuthWithSession(session);
      ...
      const ao = await createTestAO(region.id, {
        slack_channel_id: "C_AO_ONLY",
      });
      if (!ao) return;

      await linkSlackSettingsToRegion(region.id, {
        default_preblast_destination: "ao_channel",
        default_backblast_destination: "none",
      });
      ...
      expect(getSlackChannels(result)).toEqual({
        preblast: { channelId: "C_AO_ONLY", source: "ao_org_meta" },
        backblast: { channelId: "C_AO_ONLY", source: "ao_org_meta" },
      });
    });

    it("should ignore region settings that are not a plain JSON object", async () => {
      const session = await createAdminSession();
      await mockAuthWithSession(session);
      ...
      const ao = await createTestAO(region.id, {
        slack_channel_id: "C_AO_FALLBACK",
      });
      if (!ao) return;

      const slackSpace = await linkSlackSettingsToRegion(region.id, {});
      if (!slackSpace) return;

      await db
        .update(schema.slackSpaces)
        .set({ settings: sql`'["not", "an", "object"]'::jsonb` })
        .where(eq(schema.slackSpaces.id, slackSpace.id));
      ...
      expect(getSlackChannels(result)).toEqual({
        preblast: { channelId: "C_AO_FALLBACK", source: "ao_org_meta" },
        backblast: { channelId: "C_AO_FALLBACK", source: "ao_org_meta" },
      });
    });
